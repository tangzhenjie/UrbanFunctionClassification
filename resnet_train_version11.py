from data import DataGeneratorNewVersion4
from core import ResNet_final_new6
...
LOG_DIR = './log_version11/'
CHECKPOINT_DIR = './checkpoint_version11/'
NUM_CLASSES = 9
BATCHSIZE = 200
LEARNINT_RATE = 0.001
EPOCHS = 2000
...
DataGenerator = DataGeneratorNewVersion4.DataGenerator()
...
visit_array1 = tf.placeholder(tf.float32, shape=(None, 174, 26))
...
ResNetModel = ResNet_final_new6.ResNetModel(is_training, depth, NUM_CLASSES)
with tf.name_scope("ResNet"):
    fc_image = ResNetModel.inference(x)
#with tf.name_scope("visit"):
#    fc_visit = ResNet_version11.visit_network(visit_array1, is_training)
net_output = ResNet_final_new6.get_net_output(fc_image=fc_image, classNum=NUM_CLASSES, KEEP_PROB=keep_prob)
...
with tf.name_scope("train"):
    ...
    train_layers = ["scale1", "scale2", "scale3", "scale4", "scale5", "fc"]
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(EPOCHS):
        ...
        while step <= train_batches_of_epoch:
            img_batch, visit1_batch, label_batch = sess.run(next_batch)
            ...
            pre, true, _, loss_value, merge, accu = sess.run([prediction, groundtruth, train_op, loss, summary_op, accuracy], feed_dict={x: img_batch, y: label_batch, is_training: True, visit_array1: visit1_batch, keep_prob: 0.5})
        ...
        for tag in range(eval_batches_of_epoch):
            ...
            #visit1_batch = np.reshape(visit1_batch, (-1, 182, 192, 1))
            #visit1_batch = np.tile(visit1_batch, (1, 1, 1, 3))
            pre, true, acc, con_matrix = sess.run([prediction, groundtruth, accuracy,  confus_matrix], feed_dict={x: img_batch, y: label_batch, is_training: False, visit_array1: visit1_batch, keep_prob: 1.0})
