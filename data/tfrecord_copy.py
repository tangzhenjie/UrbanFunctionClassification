...
def get_data(dataset, tfrecord_path, dataset_name):
    print("Loading training set...")

    output_filename = os.path.join(tfrecord_path,
                                   dataset_name + ".tfrecord")  # /home/wangdong/桌面/工程项目目录Spyter-tensorblow/研究生竞赛 /一带一路竞赛/初赛赛题/tfrecord/train.tfrecord
    ...
    filenames = ["../OriginDataset/train/" + item[0][-7: -4] + "/" + item[0][-14:-4] + ".jpg" for item in values]                                                        # 存储方式为  ‘/train_image/008/032316_008.jpg’,‘’,‘’,''

    length = len(filenames)

    class_ids = [int(item[0][-5])-1 for item in values]     # 存储功能分类的id,从0开始到8 [0,0,0,...., 1,1,1...,2,2,2...,3,3,3...,4,4,4..,...8,8,8],每个id的个数为9342个
    for index, filename in enumerate(filenames):
        ...
        data_ = image.tobytes()
        visit_ = visit.astype(np.int32).tobytes()
        label_ = label
        example = image_to_tfexample(data_, visit_, label_)
        tfrecord_writer.write(example.SerializeToString())
        sys.stdout.write('\r>> Converting image %d/%d' % (index + 1, length))
        sys.stdout.flush()
    sys.stdout.write('\n')
    sys.stdout.flush()
    print("completed...")                                                                    # 格式是一个数组[[imgae, visit, label],[,,,],...]

def get_data_test(dataset, tfrecord_path, dataset_name):
    ...
    filenames = ["../OriginDataset/test/" + item[0].split('.')[0] + ".jpg" for item in table.values]                                                        # 存储方式为  ‘/train_image/008/032316_008.jpg’,‘’,‘’,''
    class_ids = [int(item[0].split('.')[0]) for item in table.values]     # 存储功能分类的id,从0开始到8 [0,0,0,...., 1,1,1...,2,2,2...,3,3,3...,4,4,4..,...8,8,8],每个id的个数为9342个
    length = len(filenames)
    for index, filename in enumerate(filenames):
        image = cv2.imread(filename, cv2.IMREAD_COLOR)                           # 读入一副彩色图像。图像的透明度会被忽略，(为什么透明度要被忽略，，可不可以换成)
        # image = cv2.imread(filename, cv2.IMREAD_UNCHANGED)                     # 原始图片读入
        ...
        visit_ = visit.astype(np.int32).tobytes()
        areaID_ = areaID
        example = image_to_tfexample(data_, visit_, areaID_)
        tfrecord_writer.write(example.SerializeToString())
        sys.stdout.write('\r>> Converting image %d/%d' % (index + 1, length))
        sys.stdout.flush()
    sys.stdout.write('\n')
...
if __name__ == '__main__':
    ...
    #get_data("../Dataset/train.txt", "../Dataset/tfrecord/", "train")
    #get_data("../Dataset/eval.txt", "../Dataset/tfrecord/", "eval")
    get_data_test("../Dataset/test.txt", "../Dataset/tfrecord/", "test")
