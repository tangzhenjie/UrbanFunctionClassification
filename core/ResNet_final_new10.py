...
class ResNetModel(object):
    ...
    def inference(self, x):
        ...
        with tf.variable_scope('img_scale2'):
            ...
            s2_resnet = block(x=s2_mp, is_training=self.is_training, block_filters_internal=64, block_stride=1)
        # Scale 3
        with tf.variable_scope('img_scale3'):
            s3_mp = tf.nn.max_pool(s2_resnet, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            s3_resnet = block(x=s3_mp, is_training=self.is_training, block_filters_internal=128, block_stride=1)
            # Scale 3
        with tf.variable_scope('img_scale4'):
            s4_mp = tf.nn.max_pool(s3_resnet, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            s4_resnet = block(x=s4_mp, is_training=self.is_training, block_filters_internal=256, block_stride=1)
        s5_mp = tf.nn.max_pool(s4_resnet, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        # post-net
        avg_pool = tf.reduce_mean(s5_mp, reduction_indices=[1, 2], name='avg_pool2')
...
def visit_network(visit, is_training):
    # visit shape[174, 26]
    # 对visit数据进行转换
    visit_rs = tf.reshape(visit, [-1, 174, 24, 2])
    ...
    with tf.variable_scope('visit_scale2'):
        s2_mp = tf.nn.max_pool(s1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        s2_resnet = block(x=s2_mp, is_training=is_training, block_filters_internal=64, block_stride=1)

    # Scale 3
    with tf.variable_scope('visit_scale3'):
        s3_mp = tf.nn.max_pool(s2_resnet, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        s3_resnet = block(x=s3_mp, is_training=is_training, block_filters_internal=128, block_stride=1)
    with tf.variable_scope('visit_scale4'):
        s4_mp = tf.nn.max_pool(s3_resnet, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        s4_resnet = block(x=s4_mp, is_training=is_training, block_filters_internal=256, block_stride=1)
    s5_mp = tf.nn.max_pool(s4_resnet, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    # post-net
    avg_pool = tf.reduce_mean(s5_mp, reduction_indices=[1, 2], name='avg_pool2')

    return avg_pool
def get_net_output(fc_image, fc_visit, classNum, KEEP_PROB):
    with tf.variable_scope("fc"):
        ...
        """
        #visit_drop = dropout(fc_visit, KEEP_PROB)
        #fc_image_drop = dropout(fc_image, KEEP_PROB)
        visit_image_concat = tf.concat([fc_image, fc_visit], 1, name='visit_image_concat')
        visit_image_concat_drop = dropout(visit_image_concat, KEEP_PROB)
        net_output = fc(visit_image_concat_drop, num_units_out=classNum)
    return net_output
