...
class ResNetModel(object):
    ...
    def inference(self, x):
        ...
        with tf.variable_scope('img_scale3'):
            ...
            s3_conv = conv(s3_bn, ksize=3, stride=2, filters_out=256)
...
def conv_new(x, ksize1, ksize2, stride, filters_out):
    filters_in = x.get_shape()[-1]
    shape = [ksize1, ksize2, filters_in, filters_out]
    initializer = tf.truncated_normal_initializer(stddev=CONV_WEIGHT_STDDEV)
    weights = _get_variable('weights', shape=shape, dtype='float', initializer=initializer,
                            weight_decay=CONV_WEIGHT_DECAY)
    return tf.nn.conv2d(x, weights, [1, stride, stride, 1], padding='SAME')
...
def visit_network(visit, is_training):
    ...
    with tf.variable_scope('scale1'):
        s1_conv = conv_new(visit_rs, ksize1=6, ksize2=3, stride=1, filters_out=64)
    ...
    with tf.variable_scope('scale2'):
        ...
        s2 = stack(s2_mp, is_training=is_training, num_blocks=num_blocks[0], stack_stride=1,
                   block_filters_internal=64)

    # Scale 3
    with tf.variable_scope('scale3'):
        s3 = stack(s2, is_training=is_training, num_blocks=num_blocks[1], stack_stride=2,
                   block_filters_internal=128)

    # Scale 4
    with tf.variable_scope('scale4'):
        s4 = stack(s3, is_training=is_training, num_blocks=num_blocks[2], stack_stride=2,
                   block_filters_internal=256)

    # Scale 5
    with tf.variable_scope('scale5'):
        s5 = stack(s4, is_training=is_training, num_blocks=num_blocks[3], stack_stride=2,
                   block_filters_internal=256)

    # post-net
    avg_pool = tf.reduce_mean(s5, reduction_indices=[1, 2], name='visit_pool')
